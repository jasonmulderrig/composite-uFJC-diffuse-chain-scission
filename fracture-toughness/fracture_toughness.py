...
class FractureToughnessCharacterizer(CompositeuFJCScissionCharacterizer):
    ...
    def set_user_parameters(self):
        ...
        # nu = 1 -> nu = 10000, only 121 unique nu values exist here
        nu_list = np.unique(np.rint(np.logspace(0, 4, 151)))
        check_xi_c_dot_list = np.logspace(-40, 1, 51)
    ...
    def characterization(self):
        ...
        rate_dependent_epsilon_cnu_diss_hat_crit_list = [
            [
                0. for check_xi_c_dot_indx in range(len(cp.check_xi_c_dot_list))
            ]
            for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_g_c_crit_list = [
            [
                0. for check_xi_c_dot_indx in range(len(cp.check_xi_c_dot_list))
            ]
            for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_g_c_crit__nu_squared_list = [
            [
                0. for check_xi_c_dot_indx in range(len(cp.check_xi_c_dot_list))
            ]
            for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_overline_epsilon_cnu_diss_hat_crit_list = [
            [
                0. for check_xi_c_dot_indx in range(len(cp.check_xi_c_dot_list))
            ]
            for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_overline_g_c_crit_list = [
            [
                0. for check_xi_c_dot_indx in range(len(cp.check_xi_c_dot_list))
            ]
            for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_overline_g_c_crit__nu_squared_list = [
            [
                0. for check_xi_c_dot_indx in range(len(cp.check_xi_c_dot_list))
            ]
            for nu_indx in range(len(cp.nu_list))
        ...
        for nu_indx in range(len(cp.nu_list)):
            ...
            for check_xi_c_dot_indx in range(len(cp.check_xi_c_dot_list)):
                ...
                rate_dependent_epsilon_cnu_diss_hat_crit_list[nu_indx][check_xi_c_dot_indx] = (
                    epsilon_cnu_diss_hat_crit_val
                )
                rate_dependent_g_c_crit_list[nu_indx][check_xi_c_dot_indx] = (
                    g_c_crit_val
                )
                rate_dependent_g_c_crit__nu_squared_list[nu_indx][check_xi_c_dot_indx] = (
                    g_c_crit__nu_squared_val
                )
                rate_dependent_overline_epsilon_cnu_diss_hat_crit_list[nu_indx][check_xi_c_dot_indx] = (
                    overline_epsilon_cnu_diss_hat_crit_val
                )
                rate_dependent_overline_g_c_crit_list[nu_indx][check_xi_c_dot_indx] = (
                    overline_g_c_crit_val
                )
                rate_dependent_overline_g_c_crit__nu_squared_list[nu_indx][check_xi_c_dot_indx] = (
                    overline_g_c_crit__nu_squared_val
                )
        ...
        for check_xi_c_dot_indx in range(len(cp.check_xi_c_dot_list)):
            ...
            rate_dependent_nu_fit_epsilon_cnu_diss_hat_crit_list[check_xi_c_dot_indx] = (
                epsilon_cnu_diss_hat_crit_val
            )
            rate_dependent_nu_fit_g_c_crit_list[check_xi_c_dot_indx] = (
                g_c_crit_val
            )
            rate_dependent_nu_fit_g_c_crit__nu_squared_list[check_xi_c_dot_indx] = (
                g_c_crit__nu_squared_val
            )
            rate_dependent_nu_fit_overline_epsilon_cnu_diss_hat_crit_list[check_xi_c_dot_indx] = (
                overline_epsilon_cnu_diss_hat_crit_val
            )
            rate_dependent_nu_fit_overline_g_c_crit_list[check_xi_c_dot_indx] = (
                overline_g_c_crit_val
            )
            rate_dependent_nu_fit_overline_g_c_crit__nu_squared_list[check_xi_c_dot_indx] = (
                overline_g_c_crit__nu_squared_val
            )
        
        
        rate_dependent_typcl_AFM_exprmt_epsilon_cnu_diss_hat_crit_list = [
            0. for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_typcl_AFM_exprmt_g_c_crit_list = [
            0. for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_typcl_AFM_exprmt_g_c_crit__nu_squared_list = [
            0. for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_typcl_AFM_exprmt_overline_epsilon_cnu_diss_hat_crit_list = [
            0. for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_typcl_AFM_exprmt_overline_g_c_crit_list = [
            0. for nu_indx in range(len(cp.nu_list))
        ]
        rate_dependent_typcl_AFM_exprmt_overline_g_c_crit__nu_squared_list = [
            0. for nu_indx in range(len(cp.nu_list))
        ]

        for nu_indx in range(len(cp.nu_list)):
            nu_val = cp.nu_list[nu_indx]
            f_c_dot_val = 1e1 # nN/sec
            ...
            rate_dependent_typcl_AFM_exprmt_epsilon_cnu_diss_hat_crit_list[nu_indx] = (
                epsilon_cnu_diss_hat_crit_val
            )
            rate_dependent_typcl_AFM_exprmt_g_c_crit_list[nu_indx] = (
                g_c_crit_val
            )
            rate_dependent_typcl_AFM_exprmt_g_c_crit__nu_squared_list[nu_indx] = (
                g_c_crit__nu_squared_val
            )
            rate_dependent_typcl_AFM_exprmt_overline_epsilon_cnu_diss_hat_crit_list[nu_indx] = (
                overline_epsilon_cnu_diss_hat_crit_val
            )
            rate_dependent_typcl_AFM_exprmt_overline_g_c_crit_list[nu_indx] = (
                overline_g_c_crit_val
            )
            rate_dependent_typcl_AFM_exprmt_overline_g_c_crit__nu_squared_list[nu_indx] = (
                overline_g_c_crit__nu_squared_val
            )

        save_pickle_object(
            self.savedir, rate_dependent_epsilon_cnu_diss_hat_crit_list,
            data_file_prefix+"-rate_dependent_epsilon_cnu_diss_hat_crit_list")
        save_pickle_object(
            self.savedir, rate_dependent_g_c_crit_list,
            data_file_prefix+"-rate_dependent_g_c_crit_list")
        save_pickle_object(
            self.savedir, rate_dependent_g_c_crit__nu_squared_list,
            data_file_prefix+"-rate_dependent_g_c_crit__nu_squared_list")
        save_pickle_object(
            self.savedir,
            rate_dependent_overline_epsilon_cnu_diss_hat_crit_list,
            data_file_prefix+"-rate_dependent_overline_epsilon_cnu_diss_hat_crit_list")
        save_pickle_object(
            self.savedir, rate_dependent_overline_g_c_crit_list,
            data_file_prefix+"-rate_dependent_overline_g_c_crit_list")
        save_pickle_object(
            self.savedir, rate_dependent_overline_g_c_crit__nu_squared_list,
            data_file_prefix+"-rate_dependent_overline_g_c_crit__nu_squared_list")
        ...
        save_pickle_object(
            self.savedir,
            rate_dependent_typcl_AFM_exprmt_epsilon_cnu_diss_hat_crit_list,
            data_file_prefix+"-rate_dependent_typcl_AFM_exprmt_epsilon_cnu_diss_hat_crit_list")
        save_pickle_object(
            self.savedir, rate_dependent_typcl_AFM_exprmt_g_c_crit_list,
            data_file_prefix+"-rate_dependent_typcl_AFM_exprmt_g_c_crit_list")
        save_pickle_object(
            self.savedir,
            rate_dependent_typcl_AFM_exprmt_g_c_crit__nu_squared_list,
            data_file_prefix+"-rate_dependent_typcl_AFM_exprmt_g_c_crit__nu_squared_list")
        save_pickle_object(
            self.savedir,
            rate_dependent_typcl_AFM_exprmt_overline_epsilon_cnu_diss_hat_crit_list,
            data_file_prefix+"-rate_dependent_typcl_AFM_exprmt_overline_epsilon_cnu_diss_hat_crit_list")
        save_pickle_object(
            self.savedir,
            rate_dependent_typcl_AFM_exprmt_overline_g_c_crit_list,
            data_file_prefix+"-rate_dependent_typcl_AFM_exprmt_overline_g_c_crit_list")
        save_pickle_object(
            self.savedir,
            rate_dependent_typcl_AFM_exprmt_overline_g_c_crit__nu_squared_list,
            data_file_prefix+"-rate_dependent_typcl_AFM_exprmt_overline_g_c_crit__nu_squared_list")
    ...
    def finalization(self):
        ...
        # rate_dependent_epsilon_cnu_diss_hat_crit_list = load_pickle_object(
        #     self.savedir,
        #     data_file_prefix+"-rate_dependent_epsilon_cnu_diss_hat_crit_list")
        # rate_dependent_g_c_crit_list = load_pickle_object(
        #     self.savedir,
        #     data_file_prefix+"-rate_dependent_g_c_crit_list")
        # rate_dependent_g_c_crit__nu_squared_list = load_pickle_object(
        #     self.savedir,
        #     data_file_prefix+"-rate_dependent_g_c_crit__nu_squared_list")
        # rate_dependent_overline_epsilon_cnu_diss_hat_crit_list = (
        #     load_pickle_object(
        ...
        #         data_file_prefix+"-rate_dependent_overline_epsilon_cnu_diss_hat_crit_list")
        # )
        # rate_dependent_overline_g_c_crit_list = load_pickle_object(
        #     self.savedir,
        #     data_file_prefix+"-rate_dependent_overline_g_c_crit_list")
        # rate_dependent_overline_g_c_crit__nu_squared_list = load_pickle_object(
        #     self.savedir,
        #     data_file_prefix+"-rate_dependent_overline_g_c_crit__nu_squared_list")
        ...
        #         data_file_prefix+"-rate_dependent_typcl_AFM_exprmt_epsilon_cnu_diss_hat_crit_list")
        # )
        # rate_dependent_typcl_AFM_exprmt_g_c_crit_list = load_pickle_object(
        #     self.savedir,
        #     data_file_prefix+"-rate_dependent_typcl_AFM_exprmt_g_c_crit_list")
        # rate_dependent_typcl_AFM_exprmt_g_c_crit__nu_squared_list = (
        #     load_pickle_object(
